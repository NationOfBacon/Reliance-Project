...
{
    private static EnemyDefendState _instance; //only declared once

    private EnemyDefendState() //set instance to itself
    {
        if (_instance != null)
        {
            return;
    ...
    public static EnemyDefendState instance //creates the instance of the first state if it does not exist
    {
        get
        {
            if (_instance == null)
    ...
    public override IEnumerator InState(EnemyAIMachine owner)
    {
        ...
        while (owner.botMachine.currentState == EnemyDefendState.instance)
        {
            if(!owner.disabled)
            {
                if (RotateBody(owner, point))
                {
                    ...
                    if (Vector3.Distance(owner.gameObject.transform.position, point) <= 15) //if the bot gets within 5 units of a destination, stop and wait
                    {
                ...
                {
                    owner.botMachine.ChangeState(EnemyChaseState.instance); //need a special chase and attack state so that the bot will return to defending
                    yield break;
                }
                else
                {
    ...
    public Vector3 MoveWaypoint(EnemyAIMachine _owner) //generate a position within the search radius
    {
        ...
        Vector3 point = _owner.objective.transform.position + Random.insideUnitSphere * _owner.searchRadius; //create a point on the navmesh
        NavMesh.SamplePosition(point, out NH, _owner.searchRadius, NavMesh.AllAreas); //create a position to give to the NH var

        if (Physics.Raycast(_owner.lineOrigin.transform.position, NH.position - _owner.transform.position, out hit, Vector3.Distance(_owner.lineOrigin.transform.position, point))) //draw a ray from this bot to the point at the same length as the distance from the bot to the point
        {
            //if the ray hits any object, get a new point and redo the raycast until the cast is false
            do
            {
                point = _owner.objective.transform.position + Random.insideUnitSphere * _owner.searchRadius;
                NavMesh.SamplePosition(point, out NH, _owner.searchRadius, NavMesh.AllAreas);
            }
            while (Physics.Raycast(_owner.lineOrigin.transform.position, NH.position - _owner.transform.position, out hit, Vector3.Distance(_owner.lineOrigin.transform.position, point)));
        }
